<script lang="ts">
	...
	function handleInput() {
		...
		debounceTimer = setTimeout(async () => {
			try {
				const res = await fetch(`/api/master-data/${type}?q=${encodeURIComponent(searchQuery)}`);
	...
	$effect(() => {
		if (value && !selectedName) {
			fetch(`/api/master-data/${type}?q=${encodeURIComponent(value)}`)
				.then((res) => res.json())
				.then((data) => {
					const item = data.items?.find((i: Item) => i.id === value);
					if (item) {
	...
	// Reset if query is cleared manually
	$effect(() => {
